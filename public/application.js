...
var gameBoardUi = __webpack_require__(14);
var gameHistoryEvent = __webpack_require__(4);

store.gameBoardGlobal = [];

var storeApiGame = function storeApiGame(data) {
  store.apiGame = data;
  onGameLogic();
};
...
var onCreateBoard = function onCreateBoard() {
  gameApi.createBoard().then(storeApiGame).catch(gameBoardUi.onBoardFailure);
  gameBoardUi.displayPlayerHistory();
};

var onUpdateGameData = function onUpdateGameData(data) {
  store.apiGame = data;
};

var onGameLogic = function onGameLogic() {
    ...
    if (store.gameBoardGlobal.length < 8) {
      if (!store.gameBoardGlobal.includes(store.playerPosition.toString())) {
        if (store.gameBoardGlobal.length % 2 === 0) {
          ...
          var didWin = checkForWinner('x');
          if (didWin) {
            gameApi.updateBoard([store.playerPosition - 1, 'x', false]).then(onUpdateGameData).catch(gameBoardUi.onBoardFailure);
          }
        } else {
          gameBoardUi.playerOMove('#' + store.playerPosition);
          store.gameBoardGlobal.push(store.playerPosition);
          var _didWin = checkForWinner('o');
          if (_didWin) {
            gameApi.updateBoard([store.playerPosition - 1, 'o', false]).then(onUpdateGameData).catch(gameBoardUi.onBoardFailure);
          }
        }
      } else {
...
var checkForWinner = function checkForWinner(player) {
  ...
  for (var i = start; i < store.gameBoardGlobal.length; i += 2) {
    checkBoardArray.push(store.gameBoardGlobal[i]);
  }

  for (var n = 0; n < 8; n++) {
    if (checkBoardArray.includes(winningCombos[n][0]) && checkBoardArray.includes(winningCombos[n][1]) && checkBoardArray.includes(winningCombos[n][2])) {
      gameBoardUi.gameWinner(start);
      gameApi.updateBoard([store.playerPosition - 1, player, true]).then(onUpdateGameData).catch(gameBoardUi.onBoardFailure);
      return false;
    }
  }
  return true;
...
var historyApi = __webpack_require__(15);
var historyUi = __webpack_require__(16);

var onUpdatePlayerHistory = function onUpdatePlayerHistory() {
  ...
  historyApi.findAllUserGames().then(historyCalculations).catch(historyUi.findGamesFail);
};

var historyCalculations = function historyCalculations(data) {
...
var numGamesWon = function numGamesWon(games) {
  var numXwon = 0;
  var numOwon = 0;
  for (var i = 0; i < games.length; i++) {
    ...
    if (games[i].over === true) {
      for (var n = 0; n < games[i].cells.length; n++) {
...
var numGamesNotFinished = function numGamesNotFinished(games) {
  var numOnGoingGames = 0;
  for (var i = 0; i < games.length; i++) {
...
var numTieGames = function numTieGames(games) {
  var numDraws = 0;
  for (var i = 0; i < games.length; i++) {
    if (games[i].over === false && !games[i].cells.includes('')) {
      numDraws += 1;
...
module.exports = {
  onUpdatePlayerHistory: onUpdatePlayerHistory
};
...
var historyEvents = __webpack_require__(4);
...
var gameBoardEvents = __webpack_require__(3);
...
var updateBoard = function updateBoard(dataArray) {
  return $.ajax({
    method: 'PATCH',
...
var playerXMove = function playerXMove(box) {
...
var playerOMove = function playerOMove(box) {
  $('#playerTurnX').html('Turn >> Player X');
  $('#playerTurnO').html('Player O');
  $('#playerTurnX').css('border', '1px solid white');
  $('#playerTurnO').css('border', '1px solid white');
...
var gameWinner = function gameWinner(player) {
  if (player === 0) {
...
var onBoardFailure = function onBoardFailure() {
...
var displayPlayerHistory = function displayPlayerHistory() {
  $('.view3').css('display', 'inline');
};

var alreadyPlayedSpace = function alreadyPlayedSpace() {
  if (store.gameBoardGlobal.length % 2 === 0) {
    $('#playerTurnX').css('border', '1px solid red');
...
module.exports = {
  ...
  playerOMove: playerOMove,
  gameDraw: gameDraw,
  gameWinner: gameWinner,
  resetGameBoard: resetGameBoard,
  onBoardFailure: onBoardFailure,
  displayPlayerHistory: displayPlayerHistory,
  alreadyPlayedSpace: alreadyPlayedSpace
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var findAllUserGames = function findAllUserGames() {
  return $.ajax({
    method: 'GET',
    url: config.apiUrl + '/games',
    headers: {
...
var updateUserName = function updateUserName(email) {
...
var totalGames = function totalGames(numAllGames) {
  $('#totalGames').html('Total Games Played: ' + numAllGames);
};

var showTieGames = function showTieGames(numTies) {
  $('#tieGames').html('Total Games Tied: ' + numTies);
};

var showOnGoingGames = function showOnGoingGames(onGoingGames) {
  $('#onGoingGames').html('Total Unfinished Games: ' + onGoingGames);
};

var showXandOWinners = function showXandOWinners(xWins, oWins) {
  $('#xTotalWins').html('Total X Wins: ' + xWins);
  $('#oTotalWins').html('Total O Wins: ' + oWins);
};

var findGamesFail = function findGamesFail() {
...
module.exports = {
  ...
  showTieGames: showTieGames,
  showOnGoingGames: showOnGoingGames,
  showXandOWinners: showXandOWinners,
  findGamesFail: findGamesFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
